...
Astro doesnt let you import npm modules in inline script tags within `.astro` unfortunatley. However we can do import in an external `.js`/`.ts` file, then make use of [`Astro.resolve`](https://docs.astro.build/es/reference/api-reference/#astroresolve) like so:
